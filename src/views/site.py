"""View layer — renders the static HTML dashboard from a completed pipeline run.

`render(report, dls, leads, config)` returns a single dependency-free HTML page
that explains — in plain language — how a batch of brands is discovered on
Instagram, flows through the pipeline, syncs to a CRM funnel via webhooks, and how
the reliability safeguards behave. The `build_site.py` entry point runs the
pipeline and writes the output of `render()` to `site/index.html`.
"""
...
from src.services import crm, sourcing
...
def render(report, dls, leads, config):
    s = report["summary"]
